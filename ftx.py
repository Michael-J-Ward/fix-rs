...
from socket import SHUT_RDWR, SOL_TCP, TCP_NODELAY, create_connection, socket
...
class FixConnection:
    ...
    def _get_messages(self) -> Iterator[FixMessage]:
        try:
            for msg in self._read_messages():
                logger.debug(f"connection-read-message: {msg}")
    ...
    def _read_messages(self) -> Iterator[FixMessage]:
        ...
        while True:
            try:
                ...
                logger.warning("read-message os error")
                return
            if not buf:
                return
    ...
    def send(self, values: dict) -> None:
        with self._send_lock:
            ...
            try:
                _msg = encoded.replace(b"\x01", b"|").decode()
                logger.info(f"conn-sending: {_msg}")
    ...
    def _close_on_exit(self) -> None:
        ...
        logger.info("conn._close_on_exit")
        if self.connected:
            self.close()
...
class FixClient:
    ...
    def connect(self) -> None:
        ...
        waiting = [self._connected, runner]
        logger.info(waiting)
        ready = gevent.wait(waiting, count=1)
        if runner.exception:
    ...
    def run(self) -> None:
        ...
        with ExitStack() as stack:
            sock = stack.enter_context(
                create_connection((parsed_url.hostname, parsed_url.port))
            )
            logger.info(sock)
    ...
    def send(self, values: dict) -> None:
        ...
        logger.info(f"client-sent: {values}")
    ...
    def login(self, secret: str, cancel_on_disconnect: Optional[str] = None) -> None:
        ...
        logger.info(f"logging-in: {send_time_str} {signed}")
        self.send(
            {
...
def run(api_key, secret):
    client = FixClient("tcp+ssl://fix.ftx.com:4363", target_id="FTX", client_id=api_key)
    client.login(secret)
    client.send_test_request("123")
    client.run()
    client.send_heartbeat("abcde")
    client.run()
